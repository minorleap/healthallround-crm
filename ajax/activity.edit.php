...
// Sanitise form data
	...
	if (!validate_string_length($name,0,128)) {$data_error = 2;}
	if (!validate_string_length($location,0,128)) {$data_error = 3;}
	if (!validate_string_isdate($start_date)) {$data_error = 4;}
	if (!validate_string_length($weekday,0,128)) {$data_error = 5;}
	if (!validate_string_length($start_time,0,128)) {$data_error = 6;}
	if (!validate_string_isnumber($duration_hours)) {$data_error = 7;}
	if (!validate_string_isnumber($capacity)) {$data_error = 8;}
	if (!validate_string_isnumber($frequency_id)) {$data_error = 9;}
	if (!validate_string_isnumber($has_anonymous_attendees)) {$data_error = 10;}
	if (!validate_string_isnumber($is_archived)) {$data_error = 11;}
    ...
    function generate_update_sql($table, $fields, $activity_id)
    {
    ...
    $sql = generate_update_sql("activities", $fields, $activity_id);


		// Send SQL to database
		if ($conn->query($sql) === TRUE) {
			// Record insert was successful, return primary key of new record.
			echo "OK: Activity updated successfully.";
		} else {
