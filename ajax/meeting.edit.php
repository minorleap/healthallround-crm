...
// Sanitise form data
	...
	if (!validate_string_isdate($meeting_date)) {$data_error = 3;}
	if (!validate_string_istime($meeting_time)) {$data_error = 4;}
    ...
    function generate_update_sql($table, $fields, $meeting_id)
    {
      ...
      $sql .= "WHERE id=$meeting_id;";
      return $sql;
    }
    
    $sql = generate_update_sql("activity_meetings", $fields, $meeting_id);


		// Send SQL to database
		if ($conn->query($sql) === TRUE) {
			// Record insert was successful, return primary key of new record.
			echo "OK: Meeting updated successfully.";
		} else {
