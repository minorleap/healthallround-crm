...
// Sanitise form data
	...
	$counselling_block_id = $form_data['counselling_block_id'];
	...
	if (!validate_string_isnumber($counselling_block_id)) {$data_error = 5;}

	// Check optional data is valid
	if ($evaluation_date && !validate_string_isdate($evaluation_date)) {$data_error = 6;}
    ...
    function generate_update_sql($table, $fields, $counselling_block_id)
    {
      ...
      $sql .= "WHERE id=$counselling_block_id;";
      return $sql;
    }
    
    $sql = generate_update_sql("counselling_blocks", $fields, $counselling_block_id);


		// Send SQL to database
		if ($conn->query($sql) === TRUE) {
			// Record insert was successful, return primary key of new record.
			echo "OK: Counselling block updated successfully.";
		} else {
