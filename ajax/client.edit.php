...
// Sanitise form data
	...
	if (!validate_string_length($preferred_name,0,128)) {$data_error = 100;}
	if (!validate_string_isdate($date_of_birth)) {$data_error = 4;}
	...
	if (!validate_string_isnumber($consent_data)) {$data_error = 12;}
	if (!validate_string_isnumber($consent_mail)) {$data_error = 13;}
	if (!validate_string_isnumber($consent_phone)) {$data_error = 14;}
	if (!validate_string_isnumber($consent_email)) {$data_error = 15;}
	if (!validate_string_isnumber($consent_phone_identification)) {$data_error = 16;}
	if (!validate_string_isnumber($consent_email_list)) {$data_error = 17;}
	if (!validate_string_isnumber($consent_marketing_photography)) {$data_error = 18;}
	if (!validate_string_length($preferred_contact_method,0,128)) {$data_error = 19;}
	...
	if (!validate_string_isnumber($client_id)) {$data_error = 52;}
	if (!validate_string_isnumber($transgender_id)) {$data_error = 53;}
    ...
    function generate_update_sql($table, $fields, $client_id)
    {
    ...
    $sql = generate_update_sql("clients", $fields, $client_id);
