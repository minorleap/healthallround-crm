...
// Sanitise form data
	...
	$enquiry_id = $form_data['enquiry_id'];
	...
	if (!validate_string_isnumber($user_id)) {$data_error = 2;}
	if (!validate_string_isnumber($client_id)) {$data_error = 3;}
	if (!validate_string_isdate($enquiry_date)) {$data_error = 4;}
	if (!validate_string_isnumber($service_id)) {$data_error = 5;}
	if (!validate_string_isnumber($enquiry_type_id)) {$data_error = 6;}
	if (!validate_string_isnumber($enquiry_method_id)) {$data_error = 7;}
	if (!validate_string_length($details,0,4000)) {$data_error = 8;}
	if (!validate_string_isnumber($took_enquiry_staff_id)) {$data_error = 9;}
	if (!validate_string_isnumber($passed_to_staff_id)) {$data_error = 10;}
	if (!validate_string_isnumber($time_spent_id)) {$data_error = 11;}
	if (!validate_string_isnumber($requires_feedback)) {$data_error = 12;}

	// Check optional data is valid
	if ($feedback_date && !validate_string_isdate($feedback_date)) {$data_error = 13;}
    ...
    function generate_update_sql($table, $fields, $enquiry_id)
    {
      ...
      $sql .= "WHERE id=$enquiry_id;";
      return $sql;
    }
    
    $sql = generate_update_sql("enquiries", $fields, $enquiry_id);
