...
// Sanitise form data
	...
	if (!validate_string_isnumeric($value)) {$data_error = 3;}
	if (!validate_string_isnumber($is_enabled)) {$data_error = 4;}
    ...
    $sql = generate_update_sql("duration", $fields, $record_id);


		// Send SQL to database
		if ($conn->query($sql) === TRUE) {
			// Record insert was successful, return primary key of new record.
			echo "OK: Duration updated successfully.";
		} else {
